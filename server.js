...
app.post('/api/quizzes/:id/submit', async (req, res) => {
  ...
  for (let idx = 0; idx < q.questions.length; idx++) {
    ...
    if (type === 'multiple') {
      ...
      try {
        const userAnsB64 = userAns.replace(/data:.*;base64,/, "");
        if (typeof userAns === 'string' && userAns * (6/8) - userAnsB64.replaceAll(/[^=]/g, "").length > 4000) {
        ...
        if (isImage) {
          ...
          const response = await ollama.generate({
            ...
            prompt: `Criteria: ${Array.isArray(question.answer) ? question.answer.join(", ") : question.answer}. Contents of the file: ${Buffer.from(typeof userAns === 'string' ? userAnsB64 : '', 'base64').toString('ascii')}\nis the file correct according to the criteria? `
          });
